...
int getch()
{
	//ll: the number of char in current line, reset when a new line is read
	//cc: how many characters has been read from current line. plus one when a char is get, reset when a new line is read
	if (cc == ll)
	{
		if (feof(fin))
		{
...
int block(int lev, int tx, bool* fsys)
{
	...
	do {
		...
		if (sym == constsym)    /* 收到常量声明符号，开始处理常量声明 */
		{
			...
			/* do { */
			constdeclarationdo(&tx, lev, &dx);  /* dx的值会被constdeclaration改变，使用指针 comment : this comment is wrong. dx is not changed. tx is changed. */
	...
	table[tx0].size = dx;           /* 声明部分中每增加一条声明都会给dx增加1，声明部分已经结束，dx就是当前过程数据的size 每条声明所产生的都是一个整数， 消耗内存空间相同 */
	cx0 = cx;
...
void interpret()
{
	//p: index of current instruction
	//b: zero index of instruction
	...
	do {
		...
		case opr:   /* 数学、逻辑运算 */
			...
			break;
		case lod:   /* load 取相对当前过程的数据基地址为a的内存的值到栈顶 */
			s[t] = s[base(i.l,s,b)+i.a];
			t++;
			break;
		case sto:   /* save to 栈顶的值存到相对当前过程的数据基地址为a的内存 */
			t--;
